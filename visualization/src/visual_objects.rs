...
pub fn spawn_all_entities(
    ...
    let arrow_body = meshes.add(Mesh::from(shape::Cylinder {
        radius: 0.02,
        height: 0.9,
        resolution: 16,
        segments: 1,
    }));
    let arrow_head = meshes.add(Mesh::from(Cone {
        radius: 0.04,
        height: 0.1,
        resolution: 16,
    }));
    ...
    let a = standard_material.add(emissive_material(
        Color::hex("#D6E650").unwrap(),
        Color::hex("#6B7328").unwrap(),
    ));
    let b = standard_material.add(emissive_material(
        Color::hex("#E65A50").unwrap(),
        Color::hex("#732D28").unwrap(),
    ));
    let c = standard_material.add(emissive_material(
        Color::hex("#519BE6").unwrap(),
        Color::hex("#284D73").unwrap(),
    ));
        ...
        ($transform: expr) => {
            ...
            ec.with_children(|p| {
                ...
                p.spawn(PbrBundle {
                    ...
                    material: b.clone(),
                    transform: {
                        let mut t = Transform::from_xyz(-0.15, 0.0, -0.3)
                            .with_rotation(Quat::from_rotation_y(0.7));
