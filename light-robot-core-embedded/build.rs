...
async fn main() -> anyhow::Result<()> {
    ...
    cfg.release = true;
    cfg.target = Some(PathBuf::from("../light-robot-core-frontend/index.html"));
    ...
    visit_dirs(Path::new("../light-robot-core-frontend/dist/"), &|file_path: &DirEntry| {
        ...
        create_dir_all(Path::new(&source_path.to_str().unwrap()
            .replace("light-robot-core-frontend/dist", "light-robot-core-frontend/dist-gz")).parent().unwrap()).unwrap();
        let output = File::create(Path::new(&source_path.to_str().unwrap()
            .replace("light-robot-core-frontend/dist", "light-robot-core-frontend/dist-gz"))).unwrap();
