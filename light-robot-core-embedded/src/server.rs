...
impl Server {
    ...
    where F: Fn(&Command) -> Result<()> + Send + 'static
    {
        ...
        fn serve_file<'a>(server: &'a mut EspHttpServer, path: &'static str, content: &'static [u8]) -> Result<(), EspError> {
            server.fn_handler(format!("/{}", path).as_ref(), Method::Get, move |req| -> Result<(), EspIOError> {
        ...
        server
            .fn_handler("/state", Method::Get, move |req| -> Result<(), EspIOError> {
            ...
            .fn_handler("/command", Method::Post, move |mut req| -> Result<(), EspIOError> {
                struct ReqRead<'a, A>
                {
                    req: &'a mut Request<A>,
                }
            ...
            .fn_handler("/test_result", Method::Get, move |req| -> Result<(), EspIOError> {
                let test_data = test_data.lock().unwrap().to_owned();
